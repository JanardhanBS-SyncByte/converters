...
REM Standalone Windows cmd runner for kernel-bio-converter (no .ps1).
...
set "PID_FILE=%PID_DIR%\converter-service.pid"
set "LOG_FILE=%LOG_DIR%\converter-service.log"
set "ERR_FILE=%LOG_DIR%\converter-service.err.log"
set "MODULE=kernel-bio-converter"

if not defined PORT set "PORT=8079"
if not defined SPRING_PROFILES_ACTIVE set "SPRING_PROFILES_ACTIVE=default"
if not defined IMAGE set "IMAGE=kernel-bio-converter"
if not defined loader_path_env set "loader_path_env=."
set "CONTEXT=/v1/converter-service"
...
:usage
echo Local kernel-bio-converter ^(%SPRING_PROFILES_ACTIVE% profile, :%PORT% %CONTEXT%^)
echo.
...
:print_endpoints
echo.
echo converter  profile=%SPRING_PROFILES_ACTIVE%  port=%PORT%  context=%CONTEXT%
...
echo   convert     POST %BASE%/convert
echo.
exit /b 0
...
if not errorlevel 1 (
  ...
  echo converter-service already running ^(pid !OLD_PID!^) on port %PORT%
  call :print_endpoints
  exit /b 0
)
call :port_in_use
if not errorlevel 1 (
  call :pid_on_port
  echo converter-service already listening on port %PORT% ^(pid !PORT_PID!^)
...
echo ==^> starting converter-service from %BOOT_JAR%
...
set "LAUNCH_PS1=%LOCAL_DIR%\launch-converter-service.ps1"
...
if not errorlevel 1 (
  if exist "%LOG_FILE%" (
    findstr /C:"Started KernelBioConverterApplication" "%LOG_FILE%" >nul 2>&1
...
if not defined STOP_PID (
  echo converter-service is not running
  exit /b 0
)
echo ==^> stopping converter-service ^(%STOP_PID%^) on port %PORT%
...
:smoke
call :ensure_dirs
echo ==^> waiting for converter-service ^(up to 60s^)
set /a ELAPSED=0
:smoke_loop
...
docker rm -f kernel-bio-converter >nul 2>&1
echo ==^> docker build %IMAGE%
docker build -t %IMAGE% "%MODULE_DIR%"
if errorlevel 1 exit /b 1
call :print_endpoints
docker run --rm -p %PORT%:8079 --name kernel-bio-converter -e active_profile_env=%SPRING_PROFILES_ACTIVE% -e spring_config_url_env=%SPRING_CLOUD_CONFIG_URI% -e spring_config_label_env=%SPRING_CLOUD_CONFIG_LABEL% -e JDK_JAVA_OPTIONS=%JDK_JAVA_OPTIONS% %IMAGE%
exit /b %ERRORLEVEL%
