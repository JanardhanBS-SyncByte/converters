#!/usr/bin/env bash
# kernel-bio-converter local runner — Linux, macOS, Windows Git Bash/MSYS.
...
PORT="${PORT:-8079}"
PID_FILE="${PID_DIR}/converter-service.pid"
LOG_FILE="${LOG_DIR}/converter-service.log"
MODULE="kernel-bio-converter"
...
IMAGE="${IMAGE:-kernel-bio-converter}"
CONTEXT="/v1/converter-service"
...
usage() {
  cat <<EOF
Local kernel-bio-converter (${PROFILE} profile, :${PORT} ${CONTEXT})
...
print_endpoints() {
  echo
  echo "converter  profile=${PROFILE}  port=${PORT}  context=${CONTEXT}"
...
cmd_start() {
  ...
  check_prereqs
  if is_running; then
    echo "converter-service already running (pid $(cat "$PID_FILE")) on port ${PORT}"
    print_endpoints
    return 0
  fi
  rm -f "$LOG_FILE"
  local jar
  jar="$(find_boot_jar)"
  echo "==> starting converter-service from $(basename "$jar")"
...
wait_ready() {
  ...
  while [[ "$elapsed" -lt "$timeout" ]]; do
    ...
    if grep -q "Started KernelBioConverterApplication" "$LOG_FILE" 2>/dev/null; then
...
cmd_stop() {
  ...
  local pid
  pid="$(cat "$PID_FILE")"
  echo "==> stopping converter-service (${pid}) on port ${PORT}"
...
cmd_smoke() {
  ...
  local timeout="${1:-60}"
  local elapsed=0
  echo "==> waiting for converter-service (up to ${timeout}s)"
  while [[ "$elapsed" -lt "$timeout" ]]; do
...
cmd_docker() {
  cmd_init
  need_cmd docker
  docker rm -f kernel-bio-converter >/dev/null 2>&1 || true
  echo "==> docker build ${IMAGE}"
  docker build -t "${IMAGE}" "${MODULE_DIR}"
  print_endpoints
  # shellcheck disable=SC2046
  exec docker run --rm -p "${PORT}:8079" --name kernel-bio-converter \
    $(docker_host_args) \
